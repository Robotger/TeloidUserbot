...
async def memeyap(event):
    ...
    if event.is_reply:
        ...
        if reply.photo:
            ...
            if os.path.exists("./TeloidSticker.png"):
                os.remove("./TeloidSticker.png")

            foto = await reply.download_media()
            im = Image.open(foto).convert("RGB")
            im.save("TeloidSticker.png", "png")
            Resim = "TeloidSticker.png"
        elif reply.sticker and reply.file.ext == ".tgs":
            sticker = await reply.download_media()
            os.system(f"lottie_convert.py --frame 0 -if lottie -of png '{sticker}' TeloidSticker.png")
            os.remove(sticker)
            Resim = "TeloidSticker.png"
        elif reply.media:
            Resim = await reply.download_media()
            Sure = os.system("ffmpeg -i '"+Resim+"' 2>&1 | grep Duration | awk '{print $2}' | tr -d , | awk -F ':' '{print ($3+$2*60+$1*3600)/2}'``")
            os.system(f"ffmpeg -i '{Resim}' -vcodec mjpeg -vframes 1 -an -f rawvideo -ss {Sure} TeloidThumb.jpg")
            os.remove(Resim)
            Resim = 'TeloidThumb.jpg'
        else:
            return await event.edit(LANG['REPLY_TO_MEME'])
            
        if os.path.exists("./Teloidmeme.png"):
            os.remove("./Teloidmeme.png")

        MemeYap(Resim, Text, font, Bottom, BottomText)
        await event.client.send_file(event.chat_id, "./Teloidmeme.png", reply_to=reply)
        await event.delete()
        os.remove(Resim)
    else:
